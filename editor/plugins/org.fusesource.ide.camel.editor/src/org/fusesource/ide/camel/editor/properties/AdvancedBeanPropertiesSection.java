...
public class AdvancedBeanPropertiesSection extends FusePropertySection {
	...
	private static final int PUBLIC_NON_STATIC_METHOD_BROWSE = 3;
		...
		public void widgetSelected(SelectionEvent event) {
			...
			if (control != null) {
				...
				case PUBLIC_NO_ARG_METHOD_BROWSE:
					...
					break;
				case PUBLIC_NON_STATIC_METHOD_BROWSE:
					methodName = beanConfigUtil.handlePublicNonStaticMethodBrowse(project, className,
							getDisplay().getActiveShell());
					break;
				default:
	...
	class FactoryMethodSelectionListener extends MethodSelectionListener {
		...
		public int getMethodBrowseType() {
			...
			if (beanClassCurrentValue != null && !"".equals(beanClassCurrentValue)) {
				return PUBLIC_AND_STATIC_METHOD_BROWSE;
			} else {
				return PUBLIC_NON_STATIC_METHOD_BROWSE;
			}
		}
